...
def call(String baseUrl = 'https://oxfordbrcinformatics.slack.com/services/hooks/jenkins-ci/') {
    ...
    if (jobName.lastIndexOf('/') != -1) {
        ...
        newBuildURL = buildURL.replace("job/${orgName}/job/${jName}", "blue/organizations/jenkins/${urlJobName}")
    } else {
        newBuildURL = buildURL.replace("job/${jobName}", "blue/organizations/jenkins/${jobName}")
    }
